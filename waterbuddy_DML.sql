...
-- STORED FUNCTION bereken het risico factor per klant
DROP FUNCTION IF EXISTS get_risico_factor_per_klant;

DELIMITER $$
CREATE FUNCTION get_risico_factor_per_klant(klant_id INT)
RETURNS INTEGER -- de type waarde dat deze functie returnt
READS SQL DATA -- ik ga een select statement hebben in deze functie
BEGIN
    DECLARE risico_factor DECIMAL(9,2) DEFAULT 0;
    DECLARE totaalBedrag_facturen DECIMAL(9,2);
    DECLARE totaalAantal_facturen INT;
    
    SELECT COUNT(*), SUM(factuur_bedrag)
    INTO totaalAantal_facturen, totaalBedrag_facturen
    FROM facturen f
    WHERE f.klant_id = klant_id;
    
    SET risico_factor = totaalBedrag_facturen / totaalAantal_facturen * 5;
    
    RETURN risico_factor;
END$$
